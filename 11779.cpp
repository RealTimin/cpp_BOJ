...
public:
    ...
    void Dijkstra_pq(int startIndex, int endIndex);
...
void WeightedGraph::Dijkstra_pq(int startIndex, int endIndex)
{
    struct NodeInfo
    {
        /* data */
    ...
    Node *prevNode = nullptr;
    stack<int> path;
    prevNode = nodeInfo[endIndex].prevNode;
    path.push(endIndex);
    while (prevNode != nullptr)
    {
        // cout << " -> " << prevNode->index;
        path.push(prevNode->index);
        prevNode = nodeInfo[prevNode->index].prevNode;
    }
    cout << nodeInfo[endIndex].distance << '\n';
    cout << path.size() << '\n';
    while (!path.empty())
    {
        cout << path.top() << ' ';
